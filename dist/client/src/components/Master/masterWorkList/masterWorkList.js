...
require("./masterWorkList_module.css");
var masterHeader_1 = __importDefault(require("../masterHeader/masterHeader"));
var limit = 10;
var MasterWorkList = function () {
    var masterId = (0, react_router_dom_1.useParams)().id;
    ...
    var reStatus = function (id, email) {
        if (window.confirm("confirm change status in order: " + id)) {
            axios_1["default"].put('/master/reStatus', {
                id: +id,
                email: email
      ...
      <div className="wrapper_orders">
        <table className="wrapper_orders__table">
          ...
          {orders.map(function (_a) {
            var id = _a.id, userName = _a.userName, userEmail = _a.userEmail, cityName = _a.cityName, size = _a.size, masterName = _a.masterName, startAt = _a.startAt, endAt = _a.endAt, price = _a.price, feedback = _a.feedback, rating = _a.rating, status = _a.status, email = _a.email;
            return (<tr>
                <th className="table_block_id__order">{"" + id}</th>
                ...
                <th className="table_block_id__order">{"" + status}</th>
                {status != models_1.Status.Completed ? <button type="button" onClick={function () { return reStatus(id, email); }} className="link_update__master"><th className="table_link">Сlick here to set the status completed</th></button>
                    :
...
exports["default"] = MasterWorkList;
//# sourceMappingURL=masterWorkList.js.map