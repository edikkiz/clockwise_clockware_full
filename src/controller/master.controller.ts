...
import {
    ...
    getFreeMastersForPutOrderSchema,
} from './master.shape'
...
class MasterController {
    ...
    async getFreeMasters(req: Request, res: Response) {
        ...
        if (timeToDone) {
            ...
            if (busyMastersId) {
                ...
                const x = allFreeMastersWithRating.filter(
                    elem =>
                        elem.id !=
                        allFreeMaster.find(element => element.id === elem.id)
                            ?.id,
                )
            ...
            const x = allFreeMastersWithRating.filter(
                elem =>
                    elem.id !=
                    allFreeMaster.find(element => element.id === elem.id)?.id,
            )
    ...
    async getFreeMastersForPutOrder(req: Request, res: Response) {
        ...
        if (busyMastersId.length !== 0) {
            ...
            const allFreeMastersWithRating = await prisma.$queryRaw<Master[]>`
                ...
                INNER JOIN orders ON masters.id = orders."masterId"
                WHERE masters."cityId" = ${Number(cityId)} 
                AND masters.id NOT IN (${Number(busyMastersId.join(','))})
                GROUP BY masters.id
            `
    ...
    async createMasters(req: Request, res: Response) {
        ...
        const masterValidateUpper = z
            .function()
            .args(z.string())
            .implement(arg => {
                return arg.toLowerCase() === arg
            })
        const masterValidateLower = z
            .function()
            .args(z.string())
            .implement(arg => {
        ...
        if (!PasswordWithOneSymbol.safeParse(password)) {
            validationErrors.push(
                `Password must 1 or more symbol ${'[.,/$#@&^%()*!}{}]'}`,
            )
        }
