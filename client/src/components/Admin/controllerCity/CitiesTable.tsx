...
const CitiesTable: FC<ControllerCityTableProps> = () => {
  ...
  const onSubmitDelete = useCallback((deleteId: number) => {
    setIsLoading(true)
    if (window.confirm('confirm deletion of the selected order')) {
      axios
        ...
        .then(() => {
          ...
          const deleteIndex = localCopy.findIndex(elem => elem.id === deleteId)
    ...
    <div className="wrapper_cities">
      ...
      <table className="wrapper_cities__table">
        <tr>
          ...
          <Link className="link_create__city" to="/admin/change-city" title="add new city"><th className="table_link__city">+</th></Link>
        </tr>
        {
          cities.map(({ id, name }) => (
            <tr>
              <th className="table_block_id__city">{`${id}`}</th>
              <th className="table_block_name__city">{`${name}`}</th>
              <Link to={`/admin/change-city/${id}/${name}`} title="update the city" className="link_update__city"><th className="table_link">update</th></Link>
