...
interface ControllerUserProps { }
const AllControllerUser: FC<ControllerUserProps> = () => {
  const { register, handleSubmit, setValue, formState: { errors } } = useForm<UserAdd>();

  const { id, userName, userEmail } = useParams<{ id: string, userName: string, userEmail: string }>()
  ...
  return (
    ...
    <div className="wrapper_controller-city">
      <form className="wrapper_controller-city__form" onSubmit={handleSubmit(onSubmitPut)}>
        ...
        <input className="controller_user__input" placeholder="enter user email" {...register("email", { required: true, validate: value => validator.validate(value) })}></input>
        {errors?.name?.type === FormError.REQUIRED && <p>This field is required</p>}
...
export default AllControllerUser
