...
import { City, FormError, Master } from 'src/models'
      ...
      await axios
        ...
        .then(res => {
          ...
          localStorage.setItem(
            'accessToken',
            `${res.headers.authorization.split(' ')[1]} master`,
          )
  ...
  return (
    <div className="wrapper_form">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="wrapper_form__form">
        ...
        />
        {errors?.login?.type === FormError.REQUIRED && (
          <p>This field is required</p>
        )}

        {errors?.login?.type === FormError.VALIDATE && (
        ...
        <input
          ...
          {...register('password', {
            ...
            validate: {
              ...
              testOneSymbolLowerCase: value =>
                regPasswordWithOneSymbolLowerCase.test(value),
              testOneSymbolUpperCase: value =>
                regPasswordWithOneSymbolUpperCase.test(value),
              testOneSymbol: value => regPasswordWithOneSymbol.test(value),
            },
          })}
        />
        {errors?.password?.type === FormError.REQUIRED && (
          <p>This field is required</p>
        )}

        {errors?.password?.type === FormError.TESTONENUMB && (
          <p>Password must 1 or more number </p>
        )}

        {errors?.password?.type === FormError.TESTONESYMBOLLOWERCASE && (
          <p>Password must 1 or more symbol lower case </p>
        )}

        {errors?.password?.type === FormError.TESTONESYMBOLUPPERCASE && (
          <p>Password must 1 or more symbol upper case </p>
        )}

        {errors?.password?.type === FormError.TESTONESYMBOL && (
          <p>Password must 1 or more symbol `${'[.,/$#@&^%()*!}{}]'}` </p>
        )}

        {errors?.password?.type === FormError.MINLENGTH && (
          <p>Password must 8 or more symbol </p>
        )}
        ...
        <input
          ...
          {...register('confirmPassword', {
            required: true,
            validate: value => dataForRegisterMaster[1] === value,
          })}
        />
        {errors?.confirmPassword?.type === FormError.REQUIRED && (
          <p>This field is required</p>
        )}

        {errors?.confirmPassword?.type === FormError.VALIDATE && (
        ...
        {dataForRegisterMaster[0] &&
        dataForRegisterMaster[1] &&
        dataForRegisterMaster[2] &&
        dataForRegisterMaster[3] &&
        dataForRegisterMaster[4] ? (
          <button
            className="wrapper_master_registration__form__button"
            type="submit"
          >
            Confirm{' '}
          </button>
          ...
          <button
            className="wrapper_master_registration__form__button-disabled"
            type="submit"
            disabled
          >
            Confirm
